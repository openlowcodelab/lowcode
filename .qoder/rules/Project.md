...
description: 本项目为可视化低代码平台, 旨在通过元数据驱动的方式实现应用的快速构建与渲染。其核心设计目标是将应用的结构、行为和样式抽象为可配置的元数据, 从而实现无需编写代码即可完成应用开发。
---
### 项目概述
本项目为可视化低代码平台, 旨在通过元数据驱动的方式实现应用的快速构建与渲染。其核心设计目标是将应用的结构、行为和样式抽象为可配置的元数据, 从而实现无需编写代码即可完成应用开发。

项目基于 .NET 10、Abp 和 Blazor, 采用模块化分层架构, 前端采用 Blazor Auto 模式, 后端采用 .NET 9.0 Web API 技术。
