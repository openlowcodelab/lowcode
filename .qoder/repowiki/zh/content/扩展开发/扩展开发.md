...
- 动态组件基类 LowCodeDynamicComponentBase：负责按元数据动态解析类型、渲染属性（含简单属性、RenderFragment、EventCallback），并提供旧版AntDesign到Hc组件的类型映射兼容。
...
### 动态渲染与属性/事件绑定流程
- 类型解析：优先直接解析TypeName，失败时尝试旧版AntDesign到Hc组件的映射，确保历史数据可用。
