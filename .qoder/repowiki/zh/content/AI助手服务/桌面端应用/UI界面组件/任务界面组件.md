...
- [TasksView.axaml.cs:1-20](file://src/Agent/Assistant/H.Assistant.Desktop/Views/TasksView.axaml.cs#L1-L20)
...
VM-->>V : 渲染任务卡片
U->>V : 点击“立即执行”
V->>VM : ExecuteNowCommand
VM->>API : ExecuteNowAsync(taskId)
...
### 桌面端 TasksView（Avalonia）
- 任务列表
  - 使用 UniformGrid 两列布局，ItemsSource 绑定 Tasks 集合。
  - 每个卡片显示任务名、描述、调度文本、下次执行时间（启用时可见），并提供启用开关与更多操作菜单。
- 执行记录
...
```mermaid
flowchart TD
Start(["进入任务页"]) --> LoadAgents["加载可用 Agent"]
LoadAgents --> LoadTasks["加载任务列表"]
LoadTasks --> RenderTasks{"有任务?"}
RenderTasks --> |是| ShowCards["渲染任务卡片"]
RenderTasks --> |否| EmptyState["空状态提示"]
ShowCards --> UserAction{"用户操作"}
UserAction --> |新建/编辑| OpenDialog["打开任务弹窗"]
UserAction --> |立即执行| ExecuteNow["调用执行接口"]
UserAction --> |启用/禁用| ToggleEnable["切换启用状态"]
UserAction --> |删除| DeleteTask["删除任务"]
OpenDialog --> SaveTask["保存任务"]
...
## 结论
TasksView 提供了完整的任务管理与执行记录查看能力，桌面端与 Web 端功能对齐，并通过后台任务调度子系统实现稳定可靠的触发与执行。通过合理的分页、筛选与批量操作，满足日常运维与自动化需求。未来可扩展实时推送、进度可视化与更丰富的错误重试策略。
