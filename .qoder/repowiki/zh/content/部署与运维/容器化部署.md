...
#### 配置文件分析
设计引擎的`appsettings.json`包含以下关键配置：
- **ConnectionStrings**: 数据库连接字符串，指向本地SQL Server LocalDB实例。
- **Meta**: 元数据文件路径，指向`../../../meta/apps`和`../../../meta/parts`。
- **RemoteServices**: 远程服务基地址，用于API调用。
- **Sites**: 应用站点配置，定义了`caseapp`和`testapp`的访问URL。
