...
## 简介
本文件为 AppLab 平台的整体架构文档，面向希望理解系统分层、模块化与 Blazor Web App 模式的技术读者。平台基于 .NET + ABP 框架与 Blazor（Server + WebAssembly）构建，采用“统一宿主 + 多模块”的混合架构：既支持单体部署（H.AppLab.Host.All），也支持按服务独立部署（各业务域单独 Host）。通过 ABP 模块系统实现功能解耦与独立演进；通过 HttpClient 动态代理与 WASM 懒加载，达成前端按需加载与零样板调用代码；通过多租户、认证授权、响应压缩与 AOT/Trimming 等能力，兼顾可运维性与性能。

## 项目结构
- 宿主层（Host）
  - H.AppLab.Host.All：统一宿主，聚合所有应用模块，提供统一的入口、路由、认证、多租户、中间件管线与静态资源托管。
...
- [LazyModuleServices.cs:1-144](file://src/Host/H.AppLab.Host.All/H.AppLab.Host.All.Client/LazyModuleServices.cs#L1-L144)
...
participant Host as "HostAll 进程"
...
```mermaid
graph TB
HostAll["HostAllModule"] --> Acc["Account 模块"]
HostAll --> Org["Organization 模块"]
HostAll --> Ord["Order 模块"]
HostAll --> Sys["SystemPortal 模块"]
HostAll --> Ent["Enterprise 模块"]
...
## 结论
AppLab 平台通过 ABP 模块化与 Blazor Web App（Server + WASM）的组合，实现了高内聚、低耦合、可独立部署与高性能的前后端一体化架构。统一宿主简化了部署与运维，WASM 懒加载与 HttpClient 动态代理提升了用户体验与开发效率。低代码子系统与设计/渲染分离，使平台具备强大的可扩展性与主题定制能力。结合多租户、认证授权、消息总线与缓存，平台满足企业级应用的复杂需求。
