...
- [20260801070220_CategoryTable.cs](file://src/Tools/H.Assistant.DbMigrator\Migrations\20260801070220_CategoryTable.cs)
- [CategoryEntity.cs](file://src/Agent\Assistant\H.Assistant.EntityFrameworkCore\Entities\CategoryEntity.cs)
- [AssistantDbContext.cs](file://src/Agent\Assistant\H.Assistant.EntityFrameworkCore\AssistantDbContext.cs)
- [CategoryDto.cs](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Dtos\Categories\CategoryDto.cs)
- [CategoryAppService.cs](file://src/Agent\Assistant\H.Assistant.Application\Services\CategoryAppService.cs)
- [ICategoryAppService.cs](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Services\Categories\ICategoryAppService.cs)
- [CreateCategoryDto.cs](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Dtos\Categories\CreateCategoryDto.cs)
- [UpdateCategoryDto.cs](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Dtos\Categories\UpdateCategoryDto.cs)
- [CategoryService.cs](file://src/Host\H.AppLab.Desktop\Services\CategoryService.cs)
</cite>

## 更新摘要
**所做更改**   
- 新增 Category 表迁移文档，包含完整的表结构说明
- 更新迁移脚本编写规范章节，添加新的迁移示例
- 扩展实体映射与配置说明，包含 Category 实体的完整实现
- 增加应用服务层与数据访问层的集成说明
- 更新桌面端服务集成示例
...
F["H.Assistant.DbMigrator<br/>助手域迁移"]
end
subgraph "EF Core 设计时支持"
G["OrganizationDbContextFactory.cs"]
H["EnterpriseDbContextFactory.cs"]
end
A --> B
A --> C
A --> D
...
**更新** 新增了任务分类功能的完整数据库迁移和实现，包括表结构、实体定义、应用服务和数据访问层。

#### Category 表迁移详情
- 迁移文件：`20260801070220_CategoryTable.cs`
- 表名：`Category`
- 字段结构：
  - `Id`：GUID 主键
  - `Name`：字符串，最大长度 50，非空
...
#### 实体定义与配置
- 实体类：`CategoryEntity`，继承自 `AuditedEntity<Guid>`
- 属性：`Name`（分类名称）、`Sort`（排序）
- DbContext 配置：在 `AssistantDbContext.cs` 中配置表映射和索引

#### 应用服务层实现
- 接口：`ICategoryAppService`，提供 CRUD 操作
- 实现：`CategoryAppService`，包含增删改查功能
- DTO：`CategoryDto`、`CreateCategoryDto`、`UpdateCategoryDto`

#### 桌面端集成
- 服务类：`CategoryService`，提供 ObservableCollection 绑定
- 功能：加载、添加、更新、删除分类数据
...
CategoryAppService ..|> ICategoryAppService
```

图表来源 
- [20260801070220_CategoryTable.cs:1-46](file://src/Tools/H.Assistant.DbMigrator\Migrations\20260801070220_CategoryTable.cs#L1-L46)
- [CategoryEntity.cs:1-20](file://src/Agent\Assistant\H.Assistant.EntityFrameworkCore\Entities\CategoryEntity.cs#L1-L20)
- [AssistantDbContext.cs:175-183](file://src/Agent\Assistant\H.Assistant.EntityFrameworkCore\AssistantDbContext.cs#L175-L183)
- [CategoryDto.cs:1-20](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Dtos\Categories\CategoryDto.cs#L1-L20)
- [CategoryAppService.cs:1-60](file://src/Agent\Assistant\H.Assistant.Application\Services\CategoryAppService.cs#L1-L60)
- [ICategoryAppService.cs:1-30](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Services\Categories\ICategoryAppService.cs#L1-L30)
...
- [CategoryAppService.cs:1-60](file://src/Agent\Assistant\H.Assistant.Application\Services\CategoryAppService.cs#L1-L60)
- [ICategoryAppService.cs:1-30](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Services\Categories\ICategoryAppService.cs#L1-L30)
- [CreateCategoryDto.cs:1-19](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Dtos\Categories\CreateCategoryDto.cs#L1-L19)
- [UpdateCategoryDto.cs:1-19](file://src/Agent\Assistant\H.Assistant.Application.Contracts\Dtos\Categories\UpdateCategoryDto.cs#L1-L19)
...
### 迁移脚本编写规范与最佳实践
- 命名规范：时间戳_描述（例如 20260801070220_CategoryTable.cs）
- 幂等性：避免重复执行导致错误，必要时加入条件判断
- 不可变原则：尽量只增不改，避免破坏性变更
- 注释说明：对重要变更添加清晰注释
- 版本控制：提交到源代码管理，禁止手动修改已发布迁移

**更新** 新增 Category 表迁移示例，展示了标准的表结构定义和索引约束配置。
...
**更新** 新增 Category 表的完整迁移实现示例，展示标准的数据库迁移编写模式。

#### 迁移文件结构
```csharp
// 20260801070220_CategoryTable.cs
