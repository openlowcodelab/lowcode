...
public class ChatStreamClient(IHttpClientFactory httpClientFactory, RemoteServiceOptions remoteServiceOptions)
{
    ...
    {
        var client = httpClientFactory.CreateClient(ClientServices.AssistantRemoteServiceName);
        var baseUrl = remoteServiceOptions.GetBaseUrl(ClientServices.AssistantRemoteServiceName).TrimEnd('/');
