...
public class SystemAccountAppService : ApplicationService, ISystemAccountAppService
{
    ...
    public async Task<BaseOutput<AuthResponseDto>> SystemLoginAsync(LoginRequestDto request)
    {
        ...
        if (user == null)
            return new(new AuthResponseDto { Success = false, Message = "用户名或密码错误" });

        if (!user.IsActive)
            return new(new AuthResponseDto { Success = false, Message = "账户已被禁用" });
        ...
        if (!isSystemAdmin)
            return new(new AuthResponseDto { Success = false, Message = "无权限访问" });
    ...
    public Task<BaseOutput<UserDto?>> GetCurrentUserAsync()
    {
        var httpContext = _httpContextAccessor.HttpContext;
        if (httpContext == null)
            return Task.FromResult(new BaseOutput<UserDto?>((UserDto?)null));
        ...
        return Task.FromResult(new BaseOutput<UserDto?>(dto));
    }
