...
public class WorkbenchDbContextFactory : IDesignTimeDbContextFactory<WorkbenchDbContext>
{
    public WorkbenchDbContext CreateDbContext(string[] args)
    {
        ...
        string connectionString = configuration.GetConnectionString("WorkbenchDb")!;
        var optionsBuilder = new DbContextOptionsBuilder<WorkbenchDbContext>()
            .UseSqlServer(connectionString, b => b.MigrationsAssembly(typeof(Program).Namespace));

        return new WorkbenchDbContext(optionsBuilder.Options);
    }
}
