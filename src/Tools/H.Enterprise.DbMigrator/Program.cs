...
public class Program
{
    public static async Task Main(string[] args)
    {
        ...
        using (var scope = host.Services.CreateScope())
        {
            ...
            try
            {
                var dbContext = services.GetRequiredService<EnterpriseDbContext>();

                Console.WriteLine("开始执行 Enterprise 数据库迁移...");

                // 执行数据库迁移
                await dbContext.Database.MigrateAsync();

                Console.WriteLine("Enterprise 数据库迁移完成");
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Enterprise 数据库迁移失败: {ex.Message}");
