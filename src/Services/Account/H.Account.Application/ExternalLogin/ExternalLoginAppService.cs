...
public class ExternalLoginAppService : ApplicationService, IExternalLoginAppService
{
    ...
    public async Task<BaseOutput<ExternalLoginResultDto>> ExternalLoginAsync(ExternalLoginRequestDto request)
    {
        ...
        {
            return new(new ExternalLoginResultDto { Success = false, Message = "登录信息不完整" });
        }
        ...
        if (user == null)
        {
            ...
            {
                return new(new ExternalLoginResultDto
                {
                    Success = false,
        ...
        else
        {
            ...
            if (!user.IsActive)
            {
                return new(new ExternalLoginResultDto { Success = false, Message = "账户已被禁用" });
            }
        }
    ...
    public async Task<BaseOutput<ExternalLoginResultDto>> BindExternalAccountAsync(Guid userId, ExternalLoginRequestDto request)
    {
        ...
        if (user == null)
        {
            return new(new ExternalLoginResultDto { Success = false, Message = "用户不存在" });
        }
        ...
        {
            return new(new ExternalLoginResultDto { Success = false, Message = "该外部账号已被其他用户绑定" });
        }
        ...
        {
            return new(new ExternalLoginResultDto { Success = false, Message = $"已绑定{request.Provider}账号" });
        }
    ...
    {
        var user = await _userManager.FindByIdAsync(userId.ToString());
        if (user == null) return new(new List<ExternalAccountDto>());

        var logins = await _userManager.GetLoginsAsync(user);
        return new(logins.Select(l => new ExternalAccountDto
        {
