...
public class OrgInviteAppService : ApplicationService, IOrgInviteAppService
{
    ...
    public async Task<BaseOutput<InviteDto>> CreateInviteAsync(CreateInviteDto input)
    {
        ...
        return BaseOutput<InviteDto>.Ok(new InviteDto
        {
            Token = entity.Token,
            InviteUrl = inviteUrl,
    ...
    public async Task<BaseOutput<InviteInfoDto>> GetInviteInfoAsync(string token)
    {
        ...
        if (invite == null)
            return BaseOutput<InviteInfoDto>.Ok(new InviteInfoDto { Valid = false, Message = "邀请链接无效" });

        if (invite.IsUsed)
            return BaseOutput<InviteInfoDto>.Ok(new InviteInfoDto { Valid = false, Message = "邀请链接已被使用" });

        if (invite.ExpiresAt < DateTime.UtcNow)
            return BaseOutput<InviteInfoDto>.Ok(new InviteInfoDto { Valid = false, Message = "邀请链接已过期" });

        return BaseOutput<InviteInfoDto>.Ok(new InviteInfoDto
        {
            Valid = true,
