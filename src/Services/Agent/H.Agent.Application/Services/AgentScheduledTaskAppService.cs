...
/// 定时任务管理应用服务
/// </summary>
public class AgentScheduledTaskAppService : ApplicationService, IAgentScheduledTaskAppService
{
    ...
    public async Task<PagedResultDto<ScheduledTaskDto>> GetListAsync(ScheduledTaskQueryDto input)
    {
        ...
        var dtos = tasks
            .Select(t => _objectMapper.Map<AgentScheduledTaskEntity, ScheduledTaskDto>(t))
            .ToList();
    ...
    public async Task<ScheduledTaskDto> GetAsync(Guid id)
    {
        ...
        if (task == null)
        {
            throw new EntityNotFoundException(typeof(AgentScheduledTaskEntity), id);
        }

        return _objectMapper.Map<AgentScheduledTaskEntity, ScheduledTaskDto>(task);
    }

    public async Task<ScheduledTaskDto> CreateAsync(CreateScheduledTaskInputDto input)
    {
        var task = new AgentScheduledTaskEntity
        {
    ...
    public async Task ExecuteTaskAsync(Guid taskId)
    {
        ...
        if (task == null)
        {
            throw new EntityNotFoundException(typeof(AgentScheduledTaskEntity), taskId);
        }

        await ExecuteTaskInternalAsync(task);
    }

    private async Task ExecuteTaskInternalAsync(AgentScheduledTaskEntity task)
    {
