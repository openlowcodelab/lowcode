...
/// Agent 定义管理服务实现
/// </summary>
public class AgentDefinitionAppService : ApplicationService, IAgentDefinitionAppService
{
    ...
    public AgentDefinitionAppService(
    ...
    public async Task<AgentDefinitionDto> GetAsync(Guid id)
    {
        var entity = await _agentRepository.GetAsync(id);
        return MapToDto(entity);
    }

    public async Task<PagedResultDto<AgentDefinitionDto>> GetListAsync(AgentDefinitionQueryDto input)
    {
        ...
        return new PagedResultDto<AgentDefinitionDto>(totalCount, dtos);
    }

    public async Task<AgentDefinitionDto> CreateAsync(CreateAgentDefinitionDto input)
    {
        // 检查 AgentType 是否已存在
        var query = await _agentRepository.GetQueryableAsync();
    ...
    public async Task<AgentDefinitionDto> UpdateAsync(Guid id, UpdateAgentDefinitionDto input)
    {
        var entity = await _agentRepository.GetAsync(id);
    ...
    public async Task<List<AgentDefinitionDto>> GetEnabledAgentsAsync()
    {
        var query = await _agentRepository.GetQueryableAsync();
    ...
    public async Task<List<SkillDefinitionDto>> GetAgentSkillsAsync(Guid agentId)
    {
        var agent = await _agentRepository.GetAsync(agentId);
        var skillIds = GetSkillIds(agent);
        ...
        if (!skillIds.Any())
        {
            return new List<SkillDefinitionDto>();
        }
    ...
    private static List<Guid> GetSkillIds(AgentDefinitionEntity agent)
    {
    ...
    private static AgentDefinitionDto MapToDto(AgentDefinitionEntity entity)
    {
    ...
    private static SkillDefinitionDto MapSkillToDto(SkillDefinitionEntity entity)
    {
