...
/// <summary>
/// Workbench 桌面应用插件（与 Web 端共用 Workbench 远程服务），
/// 以 "会话" 应用的形式接入 H.AppLab.Desktop 宿主外壳
/// </summary>
public sealed class WorkbenchApp : IDesktopApp
{
    ...
    {
        // 远程服务配置（RemoteServices:Workbench:BaseUrl）
        ...
        services.AddHttpClient(WorkbenchRemoteServiceName)
        ...
        // Workbench Contracts 全部 AppService 的 HTTP 代理（与 Blazor WASM 客户端一致）
        services.AddHttpClientProxies(typeof(WorkbenchApplicationContractsModule).Assembly, WorkbenchRemoteServiceName);
        ...
        // ViewModel
        services.AddSingleton<WorkbenchAppViewModel>();
    ...
    {
        return new WorkbenchAppView
        {
            DataContext = services.GetRequiredService<WorkbenchAppViewModel>()
        };
    }
}
